...
def evaluate(model,tokenizer):
    ...
    for domain in domains:
        ...
        #we compute the overall accuracy as well
        average_score += accuracy_1.item()
    print(f"Average EWOK score: {(average_score/len(domains))*100:.2f}%")
    domain_scores_full['average'] = (average_score/len(domains), average_score/len(domains))
